...
class O3DRansacDetector(BaseDetector):
    ...
    def detect_plane(pcd: o3d.geometry.PointCloud) -> (np.array, np.array):
        ...
        outliers = np.setdiff1d(np.arange(np.asarray(pcd.points).shape[0]), inliers)
    ...
    def detect_planes(self, segmented_pcd: SegmentedPointCloud) -> SegmentedPointCloud:
        ...
        for _ in range(self.num_planes):
            inliers, outliers = O3DRansacDetector.detect_plane(
                outlier_pcd
            )  # indices in small pcd with some detached planes
        ...
        outlier_indices = np.setdiff1d(
            np.arange(np.asarray(pcd.points).shape[0]), np.concatenate(detected_indices)
        )
