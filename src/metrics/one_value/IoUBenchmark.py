...
from src.utils.metrics import planes_union_indices, planes_intersection_indices, get_dictionary_indices_of_current_label
...
def iou_mean(point_cloud_predicted: np.ndarray, point_cloud_gt: np.ndarray) -> np.float64:
    plane_predicted_dict = get_dictionary_indices_of_current_label(point_cloud_predicted)
    ...
    for key, indices in enumerate(point_cloud_predicted):
        if key in plane_gt_dic:
            iou_mean_array = np.append(iou_mean_array, iou(plane_predicted_dict[key], plane_gt_dic[key]))

    return iou_mean_array.mean()


# Inheritance from OneValueBenchmark (since this abstract class is used for other metrics, while this is not possible )
class IoUBenchmark():
    def _get_metric_name(self):
        return "IoU"

    def _calculate_metric(self, point_cloud: np.array, point_cloud_predicted: np.array, point_cloud_gt: np.array) -> np.float64:
