...
def rgb_and_depth_to_pcd_custom(
    ...
    pcd, zero_depth_indices = depth_to_pcd_custom(
        depth_image, camera_intrinsics, initial_pcd_transform
    )
...
def load_rgb_colors_to_pcd(
    rgb_image, pcd: o3d.geometry.PointCloud
) -> o3d.geometry.PointCloud:
    colors = normalize_color_arr(
        rgb_image.reshape(rgb_image.shape[0] * rgb_image.shape[1], 3)
    )
...
def pcd_to_rgb_and_depth_custom(
    ...
    rgb_image = (
        colors.reshape((camera_intrinsics.height, camera_intrinsics.width, 3)) * 255
    ).astype(dtype=np.uint8)
    depth_image = (
        points[:, 2].reshape((camera_intrinsics.height, camera_intrinsics.width))
        * camera_intrinsics.factor
    )
